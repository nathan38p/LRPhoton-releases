...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QFileDialog, QTextEdit, QDoubleSpinBox, QSpinBox, QGroupBox
from matplotlib.backends.backend_qtagg import FigureCanvasQTAgg as FigureCanvas
from matplotlib.figure import Figure


class ID13CaveTab(QWidget):
    """Onglet autonome ID13 Cave. Tout le code utile est dans ce fichier."""
    ...
    def _build_ui(self):
        ...
        plot_box = QGroupBox("Prévisualisation ID13")
        ...
        self.btn_choose = QPushButton("Choisir image")
        self.btn_apply = QPushButton("Appliquer cave + beamstop")
        self.btn_save = QPushButton("Enregistrer _cave.npy")
        self.center_x = self._spin(1294.689)
        self.center_y = self._spin(1310.290)
        self.beamstop_half_height = QSpinBox()
        self.beamstop_half_height.setRange(0, 5000)
        self.beamstop_half_height.setValue(20)
        ...
        ctrl.addWidget(QLabel("Demi-hauteur beamstop (px) :")); ctrl.addWidget(self.beamstop_half_height)
        ctrl.addWidget(self.btn_apply)
        ctrl.addWidget(self.btn_save)
        self.log = QTextEdit()
        self.log.setPlainText(f"Onglet ID13 Cave\nUtilisateur : {self.user_email}")
    ...
    def choose_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Choisir image", "", "Images (*.npy *.edf);;Tous les fichiers (*)")
        if not path:
            return
        self.file_path = Path(path)
        if self.file_path.suffix.lower() == ".npy":
            self.image = np.load(self.file_path).astype(float)
        else:
    ...
    def read_edf_image(self, path):
        ...
        def get_int(keys, default):
            ...
            return default
        dim1 = get_int(["Dim_1", "Dim1"], 2167)
        dim2 = get_int(["Dim_2", "Dim2"], 2070)
    ...
    def apply_cave(self):
        ...
        # masque beamstop depuis le centre vers la droite
        half = self.beamstop_half_height.value()
        y0 = max(0, int(round(yc - half)))
        y1 = min(h, int(round(yc + half)))
        x0 = max(0, int(round(xc)))
        img[y0:y1, x0:w] = np.nan
        ...
        self.corrected = img
        self.log.append("Correction ID13 appliquée.")
        self.refresh()
    ...
    def refresh(self):
        ...
        if img is None:
            ...
            self.ax.set_title("ID13 log10(I+1)")
